...
\newcommand{\trdate}{Winter Term 2025/26}
...
A comprehensive experimental framework was developed through seven iterative phases, progressively advancing from baseline BERT embeddings with Support Vector Machines to sophisticated LLM-based few-shot classification. The research evaluates traditional machine learning approaches (SVM, Random Forest, XGBoost, LightGBM) against transformer-based models (BERT, RoBERTa, DeBERTa, multilingual variants) across six country-specific datasets encompassing English, German, Swedish, and Dutch incident reports.

Key contributions include: (1) a robust preprocessing pipeline for multilingual incident data, (2) comparative benchmarking of 10+ transformer architectures, (3) advanced class balancing strategies including SMOTE and language-aware undersampling, (4) ensemble methods combining multiple classifiers, and (5) LLM-based few-shot classification using Flan-T5 models.

Experimental results demonstrate that the baseline BERT+SVM model achieves a macro F1-score of 0.79, with subsequent iterations improving performance through multilingual embeddings and advanced machine learning techniques. The study addresses four research questions concerning classification accuracy improvements, traditional ML versus LLM comparisons, hazard type extraction, and severity prediction capabilities.

\textbf{Keywords:} Incident Classification, Process Safety, Natural Language Processing, Machine Learning, Large Language Models, Transformer Models, Multilingual Text Classification, BERT, Few-Shot Learning
...
Die Meldung von Vorfällen ist ein kritischer Prozess zur Aufrechterhaltung der Sicherheit und betrieblichen Effizienz in Branchen wie Energie, Bergbau und Gesundheitswesen. Die manuelle Klassifizierung von Vorfallberichten ist jedoch anfällig für menschliche Fehler, Inkonsistenzen und Ineffizienz. Diese Arbeit untersucht die Anwendung von maschinellem Lernen und großen Sprachmodellen (LLMs) für die automatisierte Klassifizierung von Prozesssicherheitsvorfällen in mehrsprachigen industriellen Datensätzen.

Ein umfassendes experimentelles Framework wurde durch sieben iterative Phasen entwickelt, das von grundlegenden BERT-Embeddings mit Support Vector Machines bis hin zur ausgefeilten LLM-basierten Few-Shot-Klassifikation fortschreitet. Die Forschung bewertet traditionelle maschinelle Lernansätze (SVM, Random Forest, XGBoost, LightGBM) im Vergleich zu transformerbasierten Modellen (BERT, RoBERTa, DeBERTa, mehrsprachige Varianten) über sechs länderspezifische Datensätze mit englischen, deutschen, schwedischen und niederländischen Vorfallberichten.

Die wichtigsten Beiträge umfassen: (1) eine robuste Vorverarbeitungspipeline für mehrsprachige Vorfallsdaten, (2) vergleichendes Benchmarking von mehr als 10 Transformer-Architekturen, (3) fortgeschrittene Klassenausgleichsstrategien einschließlich SMOTE und sprachbewusstem Unterabtasten, (4) Ensemble-Methoden zur Kombination mehrerer Klassifikatoren und (5) LLM-basierte Few-Shot-Klassifikation mit Flan-T5-Modellen.

Experimentelle Ergebnisse zeigen, dass das Basis-BERT+SVM-Modell einen Makro-F1-Score von 0,79 erreicht, wobei nachfolgende Iterationen die Leistung durch mehrsprachige Embeddings und fortgeschrittene maschinelle Lerntechniken verbessern. Die Studie behandelt vier Forschungsfragen bezüglich Verbesserungen der Klassifikationsgenauigkeit, Vergleiche zwischen traditionellem ML und LLMs, Extraktion von Gefahrentypen und Vorhersage des Schweregrads.

\textbf{Schlüsselwörter:} Vorfallklassifizierung, Prozesssicherheit, Verarbeitung natürlicher Sprache, Maschinelles Lernen, Große Sprachmodelle, Transformer-Modelle, Mehrsprachige Textklassifikation, BERT, Few-Shot Learning
...
I hereby declare that this thesis titled ``\textit{Categorizing Incident Reports Using Traditional Machine Learning and Large Language Models: A Comparative Study}'' and the work presented in it are my own. I confirm that:

\begin{itemize}
    \item This work was done wholly while in candidature for a Master's degree at the University of Koblenz.
...
First and foremost, I extend my deepest appreciation to my supervisors: Prof.\ Dr.\ Andreas Mauthe and Alexander Rosenbaum from the University of Koblenz, and Dr.\ Hugh Burnham-Slipper from Uniper Kraftwerke GmbH. Their expert guidance, constructive feedback, and continuous encouragement have been invaluable in shaping this research.

I am particularly grateful to Uniper Kraftwerke GmbH for providing access to the industrial incident datasets that formed the foundation of this research. The opportunity to work with real-world safety data has been instrumental in ensuring the practical relevance of this work.

I would also like to thank my colleagues and fellow researchers who provided valuable discussions and insights throughout this project.
...
This research proposes the application of machine learning and large language models (LLMs) to automate and improve the classification of text-based incident reports. The aim is to design a ML-Model that reduces errors arising from manual processes \cite{12_Silva}, improve efficiency and consistency in classification \cite{13_Bugalia}, and adapt dynamically to evolving business and regulatory requirements \cite{14_Chen}.
...
Effective incident reporting is central to risk management, yet current systems face persistent challenges. Manual classification is subject to variability in human judgment, fatigue, inconsistent criteria, reducing data reliability and weakening safety interventions \cite{Sattari2021OilGas}. As industries grow more complex, incident datasets expand in volume and linguistic diversity, amplifying classification challenges \cite{13_Bugalia}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Research Objectives} \label{chapterGeneral} 

The primary objective of this research is to address the shortcomings of manual incident reporting by developing an automated, machine learning–driven framework for incident classification. Specifically, the study aims to analyze the limitations of current manual reporting processes and identify sources of inconsistency, inefficiency, and error. Building on these insights, the research will focus on developing and training machine learning models that are capable of accurately classifying incident types and, where possible, predicting their severity. To evaluate effectiveness, the study will conduct a comparative analysis between traditional machine learning approaches and more advanced transformer-based methods, highlighting differences in accuracy, adaptability, and scalability. Finally, the framework will be validated in real-world applications to assess its practical utility, accuracy, and efficiency in supporting safety management and decision-making.

\newpage

\section{Research Questions}

Building upon the initial exploratory experiments using BERT embeddings and a linear SVM classifier, which achieved an overall accuracy of approximately 79\% and a macro-average F1-score of 0.76, the following research questions are refined to include measurable performance targets and evaluation parameters. These targets establish a clear link between experimental outcomes and the research objectives of improving incident classification reliability, scalability, and interpretability.

\begin{enumerate}
    \item \textbf{RQ1:} Can classification of \textit{Process Safety} incidents across multilingual and domain-specific datasets be improved by at least \textbf{five percentage points} in macro-average F1-score (from $\approx $0.76 to $\geq$ 0.81) compared with the current baseline BERT+SVM model?\\
   
    \item \textbf{RQ2:} How do traditional machine learning model (SVM, XGBoost, LightGBM, Random Forest, Ensemble learning) compare with transformer-based LLM models (e.g., gpt-3.5, LlaMA - 3.2) in achieving \textbf{macro-F1 $\geq$ 0.76}?\\

    \item \textbf{RQ3:} Can large language models (LLMs) be used to \textbf{extract and identify underlying hazard types from process safety incidents} (e.g., \textit{Fire/Explosion, Loss of Containment, Mechanical/Material Failure, Exposure to Toxic Substances}) from incident \textit{titles and descriptions} with at least \textbf{80\% precision} and \textbf{macro-F1 $\geq$ 0.76} across multilingual datasets?\\

    \item \textbf{RQ4:} Can a proposed classification framework predict incident severity levels (e.g., low, medium, severe) for the business case?\\ 
...
\textbf{Chapter 3: Methodology} describes the research design, including data collection and preprocessing, the proposed dataset structure, and the iterative experimental framework spanning seven development phases.

\textbf{Chapter 4: Results} presents comprehensive experimental outcomes from all iterations, including performance metrics, comparative analyses, and ablation studies.
...
\begin{enumerate}
    ...
    Achieve \textbf{macro-F1 $\geq 0.81$} on Process Safety classification (from a baseline of $\approx 0.76$), with class-level \textbf{Precision} and \textbf{Recall} reported for the Process Safety class and robustness checks across dataset shifts (country/domain).

    \item \textbf{Comparative benchmark of classical vs.\ transformer models:}
    Deliver a head-to-head benchmark showing at least one transformer-based approach (e.g., BERT/RoBERTa or GPT-derived embeddings) reaches \textbf{macro-F1 $\geq 0.80$} while maintaining \textbf{training time} and \textbf{peak memory} \emph{no worse than} the established baseline, plus an ablation of feature choices (CLS vs.\ mean pooling, document fields included) and resource–performance trade-offs.

    \item \textbf{Hazard type extraction (multilingual):}
    Implement automatic extraction of underlying hazard types from titles/descriptions with \textbf{Macro-F1 $\geq 0.76$} across multilingual corpora, with per-language reporting and error analysis on frequent confusions (e.g., Loss of Containment vs.\ Mechanical Failure).

    \item \textbf{Severity prediction:}
    Propose a multi-class severity model (low/medium/severe) achieving \textbf{Accuracy $\geq 0.70$}, \textbf{Macro-F1 $\geq 0.70$}, and \textbf{Recall$_{\text{severe}} \geq 0.80$} for the high/critical class, including calibrated probability outputs and decision thresholds suitable for risk-sensitive triage.
...
Historical incident reports will be gathered from relevant industries, ensuring diversity in data sources. The collected data exhibits several quality issues: descriptions that are overly brief, spelling mistakes, and grammatical errors. A preliminary analysis will be conducted on misspelled words to determine effective correction methods prior to applying machine learning. Additionally, the dataset includes reports in multiple languages, complicating direct integration into a single model. Therefore, non-English incident reports will be translated into English to improve consistency and results.

\section{Proposed Dataset}
\begin{enumerate}
    \item \textbf{Uniper HSSE Data:} HSSE data, Size 54000
    \item \textbf{OSHA IMIS narratives:} Domain: Process/Health, Size 90000
    \item \textbf{MSHA injury narratives:} Mining HS/PS, Size 35000
    \item \textbf{Dutch Health Complaints:} Health Safety, Size 22000
    \item \textbf{UK Environment Agency CICS summaries:} Env, Size 12000
\end{enumerate}

Additional datasets will be incorporated during the dataset splitting phase to ensure balanced and comprehensive model evaluation.
...
Before modeling, several preprocessing steps will be applied to improve data quality:
\begin{itemize}
    \item \textit{Data Anonymization:} Remove or mask any sensitive or business-critical information to protect confidentiality.
    \item \textit{Cleaning and Class balancing} Cleaning data according to standard processes (eg. NaN value removal, special characters removal etc.) after that we should also consider class balancing for classification task. 
    \item \textit{Tokenization and Lemmatization:} Break down text into tokens (e.g., words), converting all text to lowercase for uniformity. Reduce words to their base forms to normalize different word inflections.
    \item \textit{Dataset Splitting:} Divide the dataset into training and test sets.\\
\end{itemize}

Table~\ref{tab:rawdata} presents representative examples of raw incident reports drawn from industrial datasets. These examples highlight common data quality challenges encountered in real-world reporting, including multilingual descriptions, overly brief or incomplete narratives, grammatical errors, and the use of context-specific abbreviations. Such inconsistencies illustrate the complexity of preparing incident reports for automated analysis and emphasize the need for robust preprocessing techniques to ensure reliable machine learning outcomes.

\section{Model Development}
A variety of supervised machine learning algorithms such as decision trees, random forests, SVMs will be trained and evaluated for incident classification. Additionally, the approach will integrate advanced techniques to enhance performance and adaptability:

\section{Iteration 0: Baseline BERT+SVM Implementation}

\subsection{Overview and Methodology}

As a preliminary exploration of the cleaned incident data, a binary text classification experiment was conducted to predict whether an incident is a \textbf{Process Safety} case or not, using the incident's textual fields. This baseline approach combines a pre-trained transformer language model (BERT) for feature extraction with a linear Support Vector Machine (SVM) for classification. The goal of Iteration~0 is to establish a performance benchmark (Macro F1-score) against which subsequent improvements can be measured, in line with \textbf{RQ1} (which targets a 5 percentage point increase in Macro F1 over the initial baseline of $\sim$0.76).

Rather than using all available data (over 54,000 incidents in multiple languages), the baseline focuses on a monolingual subset to eliminate language variance. Specifically, only records with English text were retained. This filtering yields an \textbf{English-only dataset} of 7,920 incidents with four key fields: \texttt{CASENO} (unique case ID), \texttt{CASE\_TYPE} (incident category), \texttt{TITLE} (brief incident summary), and \texttt{CASE\_DESCRIPTION} (detailed narrative). Table~\ref{tab:category_distribution} shows the distribution of incident categories in this subset.

\begin{table}[h!]
\centering
\caption{Distribution of Incident Categories in the English-only Dataset}
\label{tab:category_distribution}
\begin{tabular}{@{}l r@{}}
\toprule
\textbf{Category} & \textbf{Number of Cases} \\
\midrule
Safety & 4,672 \\
Process Safety & 2,055 \\
Environment & 685 \\
Asset and Reputation Damage/Loss & 207 \\
Operational Loss & 193 \\
...
Clearly, the dataset is multi-class and heavily imbalanced, with \emph{Safety} incidents being the most common. \emph{Process Safety} cases account for only about 26\% of the data (2,055 out of 7,920). For the purposes of a simple binary classification demo, the focus was narrowed to distinguishing \textbf{Process Safety (PS) vs. Non-Process Safety} incidents. All categories other than "Process Safety" are grouped together as the negative class. A new binary label was created: $\texttt{binary\_label} = 1$ if $\texttt{CASE\_TYPE} = $ \emph{Process Safety}, and $0$ otherwise. Thus, in this baseline dataset, 2,055 instances are positive class (PS incidents) and 5,865 are negative class (all other types). This framing aligns with the project’s objective of identifying Process Safety events (typically the most critical incidents).

To preserve as much incident information as possible for modeling, the \texttt{TITLE} and \texttt{CASE\_DESCRIPTION} fields for each record were concatenated into a single text. The title provides a concise summary and the description provides details; combining them ensures the model can learn from the full textual context. Formally, for each incident $i$:

\begin{equation}
\text{Input}_i = \text{Title}_i \oplus \text{``.''} \oplus \text{Case\_Description}_i~,
\end{equation}

where $\oplus$ denotes string concatenation and the period adds a separator between title and description. This yields a single descriptive text per incident, which will be used as the input to the language model.

\subsubsection{Data Preparation Pipeline}

The master dataset was constructed through several preprocessing steps to consolidate multiple sources and filter down to the English subset used in Iteration~0:

\begin{enumerate}
\item \textbf{Data Loading and Integration}: Three distinct source datasets were merged to form a comprehensive master list of incidents:
\begin{itemize}
\item \textbf{Process Safety reports} (file: \texttt{RW\_ACTUALS\_PS.csv})
\item \textbf{All incidents reports} (file: \texttt{RW\_ACTUALS\_ALL.csv})
\item \textbf{Near-miss and observations} (file: \texttt{RW\_ACTUALS\_NH\_OBS.csv})
\end{itemize}
These were concatenated into a single DataFrame (aligning common columns) to yield a master list of 54,750 incident records.

We also explored creating containers in Azure Blob Storage to ingest data directly from Snowflake and thereby avoid relying on intermediate .csv exports. However, this approach was not adopted in the current pipeline because intermittent inconsistencies in the total number of incident reports likely caused by Snowflake data refresh schedules could not be fully resolved. This integration option will be revisited in future work as part of developing a robust, closed-loop data ingestion and model update workflow.  

\item \textbf{Data Type Standardization}: Column types were standardized to enforce data integrity:
\begin{itemize}
    \item Integer IDs/counts (e.g., \texttt{CASENO}, \texttt{LOCATION\_SID}, \texttt{CASES\_NO\_OF\_REGISTRATIONS}) were converted to integer type.
    \item Date fields (\texttt{CASE\_OCCURENCE\_DATE}, \texttt{CREATED\_DATE}, \texttt{MODIFIED\_DATE}, \texttt{CASE\_CLOSED\_DATE}) were parsed to proper date objects.
    \item DateTime fields (\texttt{VALID\_FROM}, \texttt{VALID\_TO}) were parsed to datetime objects.
    \item Textual/categorical fields (e.g., \texttt{TITLE}, \texttt{CASE\_DESCRIPTION}, \texttt{CASE\_TYPE}, locations, severity, etc.) were kept as strings.
\end{itemize}

\item \textbf{Data Cleaning}: Basic cleaning operations were applied:
\begin{itemize}
    \item Duplicate entries (identical \texttt{CASENO}) were dropped, keeping only the latest record per case (based on \texttt{MODIFIED\_DATE}).
    \item Records missing critical text fields (\texttt{TITLE} or \texttt{CASE\_DESCRIPTION}) were removed.
    \item Rows that were entirely empty (all fields NaN) were discarded.
    \item Column alignment: only columns common to all three source files were retained to ensure a consistent schema across merged data.
\end{itemize}

\item \textbf{Language Filtering}: To ensure linguistic consistency with the chosen model (English BERT), an automated language detection was performed on each record’s text:
\begin{itemize}
    \item Both the \texttt{TITLE} and \texttt{CASE\_DESCRIPTION} were analyzed using a language detection library.
    \item Only records where \emph{both} fields were identified as English (\texttt{'en'}) were retained.
    \item This step removed all non-English incidents (e.g., those reported in Spanish, French, etc.), yielding a monolingual subset.
\end{itemize}

\item \textbf{Feature Construction}: From the filtered English records, a minimal working dataset was created with only the essential fields for modeling: \texttt{CASENO}, \texttt{TITLE}, \texttt{CASE\_DESCRIPTION}, and \texttt{CASE\_TYPE}. The \texttt{TITLE} and \texttt{CASE\_DESCRIPTION} of each case were concatenated (with a period delimiter) to form a single text string (\texttt{text\_features}), as described above. This ensures each sample is represented by a unified text input that includes both summary and detailed description.
\end{enumerate}

The outcome of these steps is a cleansed, English-only dataset ready for NLP processing. The final dataset (\texttt{english\_dataset\_minimal.csv}) contains 7,920 records (approximately 14.47\% of the master dataset). By focusing on English text, we avoid issues with multilingual modeling in the baseline, at the cost of excluding a large portion of data.

\subsubsection{Language Detection Methodology}

To implement the language filter, the \texttt{langdetect} library was used. Both the Title and Description of each incident were checked, and records were kept only if \emph{both} fields were classified as English. The Python code snippet below illustrates this process:

\begin{lstlisting}[language=Python, caption=Language Detection Implementation]
from langdetect import detect, LangDetectException

def detect_language(text):
    """
    Detect language of text. Returns 'en' for English.
...
# Determine language for Title and Description fields
master_df['title_lang'] = master_df['TITLE'].apply(detect_language)
master_df['description_lang'] = master_df['CASE_DESCRIPTION'].apply(detect_language)

# Filter dataset to only English entries (both Title and Description)
english_df = master_df[
    (master_df['title_lang'] == 'en') &
    (master_df['description_lang'] == 'en')
].copy()

# Create a minimal dataset with key fields
english_dataset_minimal = english_df[
    ['CASENO', 'TITLE', 'CASE_DESCRIPTION', 'CASE_TYPE']
].copy()

# Save the English-only dataset (for reuse in modeling)
english_dataset_minimal.to_csv('english_dataset_minimal.csv', index=False)
\end{lstlisting}

Applying this filter dramatically reduced the dataset size. Table~\ref{tab:language-detection} summarizes the effect of language filtering:

\begin{table}[h]
\centering
\caption{Language Detection Results}
\label{tab:language-detection}
\begin{tabular}{lrc}
\toprule
\textbf{Metric} & \textbf{Count} & \textbf{Percentage} \\
\midrule
Total Master Dataset Records & 54,750 & 100\% \\
English Records (Both Fields) & 7,920 & 14.47\% \\
Non-English Records (Excluded) & 46,830 & 85.53\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Implications of Language Filtering:}

\begin{itemize}
\item \textbf{Model Compatibility}: Restricting to English ensures all text can be processed by the BERT \emph{base-uncased} model (which is English-only).
\item \textbf{Data Reduction}: Approximately \textbf{85.5\%} of the total records were excluded for not being English. This is a significant loss of data.
\item \textbf{Geographic Bias}: English-language reports are likely biased towards regions like the UK, North America, and some international operations.
\item \textbf{Need for Multilingual Capability}: The large proportion of excluded data underlines the importance of extending the model to multiple languages in future iterations.
\item \textbf{Baseline Validity}: The English-only subset provides a controlled baseline to prove the concept. The baseline results are still meaningful, but they represent a lower bound on performance since much of the data were set aside.
\end{itemize}

After filtering, the English dataset retains the multi-class category distribution shown earlier in Table~\ref{tab:case_distribution}. As mentioned, we collapse this to a binary outcome for modeling: \textbf{Process Safety (positive class)} versus \textbf{Non-Process Safety (negative class)}.

\subsubsection{Binary Classification Formulation}

Formally, the classification task is defined as follows:

\begin{equation}
y_i =
\begin{cases}
1, & \text{if CASE\_TYPE}_i = \text{``Process Safety''},\\
0, & \text{otherwise (Non-Process Safety).}
\end{cases}
\end{equation}

In other words, if an incident’s category is \emph{Process Safety}, we label it as 1; any other category is labeled 0. This yields 2,055 positive examples and 5,865 negative examples in the dataset.

\subsection{BERT-base-Uncased for Text Embeddings}

To transform the incident texts into numerical features for the classifier, we leveraged the pre-trained \textbf{BERT-base-uncased} language model \cite{devlin2018bert}. BERT (Bidirectional Encoder Representations from Transformers) is a transformer model that produces contextual embeddings for text. Using BERT’s embeddings provides a rich, 768-dimensional representation of each incident’s text.

\subsubsection{Model Specifications}

We used the base version of BERT (uncased, English):

\begin{itemize}
\item 12 Transformer encoder layers, 12 self-attention heads, hidden size 768.
\item Vocabulary: 30,522 WordPiece tokens (all lowercase).
\item Input formatting: [CLS] at start, [SEP] at end, max length 512 tokens.
\item Pre-training corpora: BooksCorpus and English Wikipedia.
\item Uncased model: all text is lowercased before tokenization.
\end{itemize}

\paragraph{Embedding Generation Pipeline} For each incident’s combined text, we extracted the [CLS] embedding from the last BERT layer:

\begin{equation}
\mathbf{h}_{\text{[CLS]}} = \text{BERT}_{L=12}\big([\texttt{[CLS]}] \oplus \text{tokens} \oplus [\texttt{[SEP]}]\big)[0] \in \mathbb{R}^{768}.
\end{equation}

The following code was used:

\begin{lstlisting}[language=Python, caption=BERT Embedding Extraction]
from transformers import AutoTokenizer, AutoModel
import torch
from tqdm import tqdm
import numpy as np

# Initialize tokenizer and model for BERT-base-uncased
tokenizer = AutoTokenizer.from_pretrained('bert-base-uncased')
model = AutoModel.from_pretrained('bert-base-uncased')

def get_bert_embeddings(texts, max_length=512):
    """
    Convert a list of text strings into BERT [CLS] embeddings.
    Returns a NumPy array of shape (len(texts), 768).
    """
    embeddings = []
    batch_size = 8  # use small batches to fit in memory
    for i in tqdm(range(0, len(texts), batch_size),
        ...
        # Forward pass without gradient calculation (inference mode)
        with torch.no_grad():
...
# Prepare the input texts (concatenated Title and Description)
texts = english_dataset_minimal['TITLE'] + '. ' + \
        english_dataset_minimal['CASE_DESCRIPTION']
X = get_bert_embeddings(texts.tolist())  # shape: (7920, 768)
\end{lstlisting}

\subsubsection{Classification Model: Linear Support Vector Machine}

With BERT embeddings computed, we trained a \textbf{linear SVM}:

\begin{itemize}
\item Kernel: linear
\item Regularization parameter: $C = 1.0$
\item Class weighting: \texttt{class\_weight = 'balanced'}
\item Random state: 42
\end{itemize}

We used an 80/20 stratified train/test split.

The optimization objective is:

\begin{equation}
\begin{aligned}
\min_{\mathbf{w}, b} \quad & \frac{1}{2}\lVert\mathbf{w}\rVert^2 + C \sum_{i=1}^{N} w_{y_i}\,\xi_i \\
\text{s.t.} \quad & y_i(\mathbf{w}^\top \mathbf{x}_i + b) \ge 1 - \xi_i, \quad \xi_i \ge 0,\; i = 1,\dots,N~,
\end{aligned}
\end{equation}

where $w_{y_i}$ are the class weights.
...
\begin{table}[h]
\centering
\caption{Iteration 0 Dataset Distribution and Statistics}
\label{tab:iter0-dataset-stats}
\begin{tabular}{lrr}
\toprule
\textbf{Characteristic} & \textbf{Count} & \textbf{Percentage} \\
\midrule
\textbf{Total Samples (English subset)} & 7,920 & 100.00\% \\
\quad Training Samples & 6,336 & 80.00\% \\
\quad Testing Samples & 1,584 & 20.00\% \\
\midrule
\textbf{Training Set Distribution} & & \\
\quad Non-Process Safety & 4,692 & 74.05\% \\
\quad Process Safety & 1,644 & 25.95\% \\
\midrule
\textbf{Testing Set Distribution} & & \\
\quad Non-Process Safety & 1,173 & 74.05\% \\
\quad Process Safety & 411 & 25.95\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Iteration 0 Performance Metrics (BERT-base-uncased + Linear SVM)}
\label{tab:iter0-metrics}
\begin{tabular}{lccc}
\toprule
...
\midrule
Support (Test Samples) & 1,173 & 411 & 1,584 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Iteration 0 Confusion Matrix (Test Set, $n=1584$)}
\label{tab:iter0-confusion}
\begin{tabular}{lcc}
\toprule
\textbf{Actual $\downarrow$ / Predicted $\rightarrow$} & \textbf{Non-PS} & \textbf{Process Safety} \\
\midrule
\textbf{Non-Process Safety} & 964 \textit{(True Negative)} & 209 \textit{(False Positive)} \\
\textbf{Process Safety}     & 72 \textit{(False Negative)} & 339 \textit{(True Positive)} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Iteration 0 Summary of Test Performance}
\label{tab:iter0-summary}
\begin{tabular}{lc}
\toprule
\textbf{Metric} & \textbf{Value} \\
\midrule
Overall Accuracy       & 0.8226 (82.26\%) \\
\textbf{Macro F1-Score} & \textbf{0.7899} \\
...
\midrule
False Positive Rate (FPR) & 17.82\% \\
False Negative Rate (FNR) & 17.52\% \\
\midrule
Specificity (Non-PS recall) & 0.8218 \\
Sensitivity (PS recall)     & 0.8248 \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Gap to RQ1 Target}

\begin{table}[h]
\centering
\caption{Performance Gap Analysis for RQ1}
\label{tab:iter0-gap}
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{Current (Iter~0)} & \textbf{RQ1 Target} & \textbf{Gap} \\
\midrule
Macro F1-Score & 0.7899 & $\geq$ 0.8100 & +0.0201 \\
\bottomrule
\end{tabular}
\end{table}

(Discussion sections omitted here for brevity; you can keep your existing text as-is around these tables and equations.)

\section{Further Implementation and Testing}
...
\begin{itemize}
    \item \texttt{intfloat/multilingual-e5-base}: Text embedding model optimized for semantic similarity across 100+ languages
...
1 & 0.8872 & 0.8156 & \checkmark & Country stratification \\
2 & 0.8760 & 0.8120 & \checkmark & Multi-model comparison \\
3 & 0.8889 & 0.8234 & \checkmark & Multilingual embeddings \\
4 & \textbf{0.8923} & \textbf{0.8612} & \checkmark & Advanced ML techniques \\
5 & 0.8234 & 0.7856 & \checkmark & Aggressive SMOTE \\
...
\midrule
Linear SVM + BERT & 0.7899 & \checkmark \\
XGBoost + RoBERTa & 0.8834 & \checkmark \\
LightGBM + E5-large & 0.8856 & \checkmark \\
Flan-T5-Large (few-shot) & 0.0135 & $\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
RQ1: Improve PS classification by 5+ points macro F1 & $\geq 0.81$ & \textbf{0.8923} \checkmark \\
RQ2: Compare Traditional ML vs. LLM & -- & ML > LLM \checkmark \\
RQ3: Hazard extraction with 80\% precision & $\geq 0.80$ & 0.12 $\times$ \\
RQ4: Severity prediction & $\geq 0.70$ & Not addressed \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Severity Prediction:} Multi-class severity classification remains unaddressed
...
\begin{center}
\texttt{https://github.com/[repository-url]}
\end{center}
