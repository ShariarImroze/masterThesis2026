...
The generated embedding matrices reflected the size of each English subset while preserving a fixed embedding dimensionality of 768. Specifically, the manually constructed subset produced an embedding matrix of size $(4713, 768)$, the \texttt{langdetect}-based subset yielded a matrix of size $(5989, 768)$, and the \texttt{lingua}-based subset produced a matrix of size $(6222, 768)$. These dimensions confirm that each incident report was represented by a 768-dimensional dense vector derived from the underlying BERT model.
...
langdetect & 4,791 & 1,198 & 1,222 / 3,569 & 306 / 892 \\
...
% IMPORTANT:
% The current notebook output and the saved Iteration~0 report files are not yet fully synchronized.
% The data-construction subsections above now follow the latest notebook output
% (manual = 4,713; langdetect = 5,989; lingua = 6,222),
% whereas the saved classification reports and confusion matrices still reflect an older result state.
% Before final submission, rerun Iteration 0 and regenerate:
% 1) classification reports,
% 2) confusion matrices,
% 3) any result summary tables,
% so that Sections 3.5.7--3.5.10 follow a single authoritative source.
...
langdetect & 5,989 & 0.8122 & 0.7636 & 0.8160 \\
...
\multirow{2}{*}{langdetect}
    & Non-Process Safety & 0.8928 & 0.8498 & 0.8708 & \multirow{2}{*}{0.7636} & 892 \\
    & Process Safety     & 0.6160 & 0.7026 & 0.6565 &  & 306 \\
\midrule
\multirow{2}{*}{lingua}
...
\begin{figure}[p]
    ...
    \begin{subfigure}[b]{0.82\textwidth}
        ...
        \caption{Langdetect English subset: TN=758, FP=134, FN=91, TP=215.}
...
For the \texttt{langdetect} subset, the classifier correctly identifies 758 Non-Process Safety and 215 Process Safety reports, while producing 134 false positives and 91 false negatives.
...
\begin{itemize}
    ...
    \item Performance on the non-English datasets was substantially weaker, particularly for Sweden (macro-F1 = 0.5829) and Germany (macro-F1 = 0.5983).
...
Sweden      & 12,224 & 0.7951 & 0.5829 & 0.3548 \\
...
The official Iteration~0 baseline, based on the manually curated English subset, achieved a macro F1-score of 0.7825.

\begin{itemize}
    \item Iteration 1 (country stratification): 0.7449 macro F1 on the best-performing UK dataset
    \item Iteration 3 (multilingual): 0.8889 macro F1
    \item Iteration 4 (advanced ML): \textbf{0.8923 macro F1}
\end{itemize}

The best-performing configuration achieved a \textbf{10.97 percentage point improvement} over the official Iteration~0 baseline of 0.7825, thereby exceeding the RQ1 target of 0.85.
...
\begin{enumerate}
    ...
    \item Gradient boosting classifiers (LightGBM, XGBoost) outperform linear SVM
\end{enumerate}
