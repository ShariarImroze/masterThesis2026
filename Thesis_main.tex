...
Key contributions include: (1) a robust preprocessing pipeline for multilingual incident data, (2) comparative benchmarking of more than 10 transformer architectures, (3) advanced class balancing strategies including SMOTE and language-aware undersampling, (4) ensemble methods combining multiple classifiers, and (5) LLM-based few-shot classification using Flan-T5 models.
...
\begin{enumerate}
    ...
    Propose a multi-class severity model (low/medium/severe) for the business case. 
...
\begin{enumerate}
    ...
    %\item \textbf{Dutch Health Complaints:} Health Safety, Size 22000 /not needed
    %\item \textbf{UK Environment Agency CICS summaries:} Env, Size 12000 /not needed
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textit{Cleaning and Class balancing} Cleaning data according to standard processes (eg. NaN value removal, special characters removal etc.) after that consider class balancing for the classification task was considered. 
...
After filtering, the English dataset retains the multi-class category distribution shown earlier in Table~\ref{tab:case_distribution}. As mentioned, this resultd in a binary outcome for modeling: \textbf{Process Safety (positive class)} versus \textbf{Non-Process Safety (negative class)}.
...
In other words, if an incident’s category is \emph{Process Safety}, it is labelled as 1; any other category is labeled 0. This yields 2,055 positive examples and 5,865 negative examples in the dataset.

\subsection{BERT-base-Uncased for Text Embeddings}

To transform the incident texts into numerical features for the classifier, a pre-trained \textbf{BERT-base-uncased} language model \cite{devlin2018bert} was used. BERT (Bidirectional Encoder Representations from Transformers) is a transformer model that produces contextual embeddings for text. Using BERT’s embeddings provides a rich, 768-dimensional representation of each incident’s text.

\subsubsection{Model Specifications}

The base version of BERT (uncased, English) was used:
...
\paragraph{Embedding Generation Pipeline} For each incident’s combined text, the [CLS] embedding is extracted from the last BERT layer:
...
With BERT embeddings computed, the next task was to train a \textbf{linear SVM} with the following parameters: Kernel: linear, Regularization parameter: $C = 1.0$, Class weighting: \texttt{class\_weight = 'balanced'}, Random state: 42. 
...
RQ4: Severity prediction &  & Not addressed \\
\bottomrule
\end{tabular}
\end{table}
